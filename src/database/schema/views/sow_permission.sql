...
SELECT 
        teacher_join.auth_user_id as teacher_id,
        ...
        IF(dep.head_id = teach.id, True, teacher_join.is_authorised) as is_authorised, -- the head of department is immediately approved
        sow_teach.join_code as join_code
    ...
    LEFT JOIN sow_scheme_of_work__has__teacher as sow_teach 
		ON sow_teach.join_code = dep_teach.join_code -- link schemes of work to department teachers
		LEFT JOIN sow_scheme_of_work as sow ON sow.id = sow_teach.scheme_of_work_id
			INNER JOIN sow_teacher_join_code as teacher_join
				ON teacher_join.join_code = sow_teach.join_code

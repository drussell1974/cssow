CREATE TABLE `sow_department__has__teacher` (
  ...
  `teach.scheme_of_work_permission` SMALLINT NOT NULL DEFAULT 64,
  `teach.lesson_permission` SMALLINT NOT NULL DEFAULT 64,
  `teach.department_permission` SMALLINT NOT NULL DEFAULT 64,
  PRIMARY KEY (`auth_user_id`, `department_id`),
  INDEX `fk_sow_department__has__teacher__has__department_id` (`department_id` ASC) VISIBLE,

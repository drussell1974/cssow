...
class LessonScheduleModel(BaseModel):
    ...
    def get_all(db, lesson_id, scheme_of_work_id, auth_user, show_next_days=0, fn_resolve_url=None):
        ...
        from_date = auth_user.academic_year.start_date if show_next_days == 0 and auth_user.academic_year is not None else datetime.today().date()
        to_date = auth_user.academic_year.end_date if show_next_days == 0  and auth_user.academic_year is not None else datetime.today().date() + timedelta(show_next_days)

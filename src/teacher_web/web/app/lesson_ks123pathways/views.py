...
from ..lesson_ks123pathways.viewmodels import LessonKS123PathwaySelectViewModel #, LessonKS123PathwayGetModelViewModel, LessonKS123PathwayIndexViewModel, LessonKS123PathwaySaveViewModel, LessonKS123PathwayDeleteUnpublishedViewModel
...
def select(request, institute_id, department_id, scheme_of_work_id, lesson_id, auth_ctx):
    ...
    wizard = WizardHelper(
        next_url=reverse('learningobjective.new', args=[institute_id, department_id, scheme_of_work_id, lesson_id]),
    ...
    pathways_select = LessonKS123PathwaySelectViewModel(db=db, request=request, lesson_id=lesson_id, scheme_of_work_id=scheme_of_work_id, auth_user=auth_ctx, wizard=wizard)  
    
    if request.method == "POST":
        
        pathways_select.execute(request)

        if pathways_select.saved == True:

            redirect_to_url = wizard.get_redirect_url(request)

            return HttpResponseRedirect(redirect_to_url)

    return render(request, "lesson_ks123pathway/select.html", pathways_select.view(request).content)
...
def new(request, institute_id, department_id, scheme_of_work_id, lesson_id, auth_ctx):
    ...
    view_model = ViewModel(request, lesson.title, lesson.title, "Create new keyword for %s" % lesson.title, ctx=auth_ctx, data=data)
...
def edit(request, institute_id, department_id, scheme_of_work_id, lesson_id, keyword_id, auth_ctx):
    ...
    view_model = ViewModel(request, lesson.title, lesson.title, "Edit: {} for {}".format(model.term, lesson.title), ctx=auth_ctx, data=data, active_model=model, alert_message="")
...
def save(request, institute_id, department_id, scheme_of_work_id, lesson_id, keyword_id, auth_ctx):
    ...
    if model.is_valid == True:
        ...
        view_model = ViewModel(request, lesson.title, lesson.title, sub_heading, ctx=auth_ctx, data=data, active_model=model, alert_message="", error_message=error_message)

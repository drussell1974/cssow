...
class SchemeOfWorkIndexViewModel(BaseViewModel):
    ...
    def view(self, request):
        ...
        return ViewModel(request, "", "Schemes of Work", "Our shared schemes of work by key stage", ctx=self.auth_user, data=data)
...
class SchemeOfWorkEditViewModel(BaseViewModel):
    ...
    def view(self, request):
        ...
        return ViewModel(request, "", "Schemes of Work", self.model.name if len(self.model.name) != 0 else "Create new scheme of work", ctx=self.auth_user, data=data, active_model=self.model, stack_trace=self.stack_trace, error_message=self.error_message, alert_message=self.alert_message, delete_dialog_message=delete_message, wizard=self.wizard)

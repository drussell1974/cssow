...
class SchemeOfWorkViewSet(APIView):
    ...
    def get(self, request, scheme_of_work_id):
        #253 check user id
        schemeofwork_view = SchemeOfWorkGetModelViewModel(db=db, scheme_of_work_id=scheme_of_work_id, auth_user=auth_user_model(db, request))
...
class SchemeOfWorkListViewSet(APIView):
    ...
    def get (self, request):
        #253 check user id
        schemesofwork_view = SchemeOfWorkGetAllViewModel(db=db, auth_user=auth_user_model(db, request))

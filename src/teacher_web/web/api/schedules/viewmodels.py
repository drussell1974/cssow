...
class LessonScheduleViewModel(BaseViewModel):
    ...
    def __init__(self, db, scheme_of_work_id, lesson_id, show_next_days, auth_ctx, fn_resolve_url):
        self.db = db

        # get list
        data = LessonScheduleModel.get_all(self.db, scheme_of_work_id=scheme_of_work_id, lesson_id=lesson_id, show_next_days=show_next_days, auth_user=auth_ctx, fn_resolve_url=fn_resolve_url)

...
def fake_academic_year():
    return AcademicYearContextModel.default().__dict__
...
def fake_academic_year_periods():
    return [
        AcademicYearPeriodContextModel("09:00", "Period 1", is_from_db=True).__dict__, 
        AcademicYearPeriodContextModel("10:00", "Period 2", is_from_db=True).__dict__, 
        AcademicYearPeriodContextModel("11:15", "Period 3", is_from_db=True).__dict__, 
    ]
    ...
    with patch.object(InstituteContextModel, "get_context_model", return_value = InstituteContextModel(127671276711, name="Lorum Ipsum")) as institute:
        with patch.object(DepartmentContextModel, "get_context_model", return_value = DepartmentContextModel(67, name="Computer Science", is_from_db=True)) as department:
            with patch.object(SchemeOfWorkContextModel, "get_context_model", return_value = SchemeOfWorkContextModel(67, name="Nunc maximus purus", is_from_db=True)) as scheme_of_work:
                with patch.object(AcademicYearContextModel, "cached", return_value = fake_academic_year()) as academic_year:
                    with patch.object(AcademicYearContextModel, "cached_array", return_value = fake_academic_years()) as academic_year:
                        with patch.object(AcademicYearPeriodContextModel, "cached_array", return_value = fake_academic_year_periods()) as academic_year:

...
class test_db__get_all(TestCase):
    ...
    def test__should_call_select_return_single_item(self):
        # arrange
        expected_result = [
            (1029, "2020-08-23 03:49:56", "ERROR", "An error occured doing some stuff", "nec arcu nec dolor vehicula ornare non.", "A", "B"),
            ]
        ...
        with patch.object(ExecHelper, 'select', return_value=expected_result):
            ...
            self.assertEqual("ERROR", actual_results[0].type),
            ...
            self.assertEqual("nec arcu nec dolor vehicula ornare non.", actual_results[0].detail)
    ...
    def test__should_call_select_return_multiple_item(self):
        # arrange
        expected_result = [
            (1021, "2020-07-27 15:48:40", "ERROR", "An error occured doing stuff", "nec arcu nec dolor vehicula ornare non.", "X", "Y"),
            (1022, "2020-07-27 15:51:08", "ERROR", "An error occured doing same thing", "purus lacus, ut volutpat nibh euismod.", "Y", "Z"),
            (1023, "2020-08-23 03:48:01", "WARNING", "Validation errors", "rutrum lorem a arcu ultrices, id mollis", "Z", "A")
        ]
        ...
        with patch.object(ExecHelper, 'select', return_value=expected_result):
            ...
            self.assertEqual("WARNING", actual_results[2].type),
            self.assertEqual("2020-08-23 03:48:01", actual_results[2].created),
            self.assertEqual("Validation errors", actual_results[2].message)
            self.assertEqual("rutrum lorem a arcu ultrices, id mollis", actual_results[2].detail)

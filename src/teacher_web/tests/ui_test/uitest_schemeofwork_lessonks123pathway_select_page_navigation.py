...
class uitest_schemeofwork_lessonks123pathway_select_page_navigation(UITestCase):

    test_context = WebBrowserContext()

    def setUp(self):
        # setup
        self.do_log_in(self.root_uri + f"/institute/{self.test_institute_id}/department/{self.test_department_id}/schemesofwork/{self.test_scheme_of_work_id}/lessons/{self.test_lesson_id}/pathways/select")
    ...
    def test_page__breadcrumb__navigate_to_keywords_index(self):
        #test
        elem = self.test_context.find_element_by_id('btn-bc-ks123pathways')
